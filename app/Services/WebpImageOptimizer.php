...
class WebpImageOptimizer
{
    ...
    public function optimizeUploadedFile($uploadedFile, $directory, array $options = [])
    {
        ...
        if ($mime === 'image/gif') {
            ...
            return $this->result($directory . '/' . $fileName, $originalName, $originalSize, filesize($absolutePath), false, 'gif');
        }
        ...
        if ($this->supportsWebp()) {
            ...
            return $this->result($directory . '/' . $fileName, $originalName, $originalSize, filesize($absolutePath), true, 'webp');
        }
        ...
        $uploadedFile->move($absoluteDirectory, $fileName);
        $absolutePath = $absoluteDirectory . DIRECTORY_SEPARATOR . $fileName;
        return $this->result($directory . '/' . $fileName, $originalName, $originalSize, filesize($absolutePath), false, $extension);
    }
    ...
    private function encodeToWebp($sourcePath, $targetPath, $mime, array $options)
    {
        ...
        if ($contents === false) throw new Exception('No fue posible leer la imagen para optimizarla.');
        $image = @imagecreatefromstring($contents);
        if (!$image) throw new Exception('La imagen está dañada o el servidor no reconoce su formato.');
    ...
    {
        try { return bin2hex(random_bytes(5)); }
        catch (Exception $e) { return str_replace('.', '', uniqid('', true)); }
    }
